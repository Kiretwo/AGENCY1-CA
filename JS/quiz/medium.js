...
function endQuiz() {
    ...
    const quizResultMedium = { //lager en const med rsultat for quiz som videre lagres i local storage
        correctAnswers: correctAnswers, totalQuestions: quizData.length, timeSpent: elapsedTime, date: new Date().toLocaleString() 
    };

    localStorage.setItem('quizResultMedium', JSON.stringify(quizResultMedium));

    const previousResult = JSON.parse(localStorage.getItem('quizResultMedium'));
