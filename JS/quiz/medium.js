...
function endQuiz() {
    ...
    const quizResultMedium = { correctAnswers: correctAnswers, totalQuestions: quizData.length, timeSpent: elapsedTime, date: new Date().toLocaleString()};

  
    const results = JSON.parse(localStorage.getItem('quizResultsMedium')) || [];   // henter tidligere resultater eller oppretter et nytt array
    ...
    results.push(quizResultMedium);
   
    localStorage.setItem('quizResultsMedium', JSON.stringify(results));
    ...
    if (previousResult) { //sjekker om det finnes et tidligere resultat å vise
        resultContainer.innerHTML += `<div class="previous-results">
                                        <h3>Previous result on medium</h3>
